...
variable "private_key_path" {
  default = "keys/environment_name"
}

# The public key.
variable "public_key_path" {
  default = "keys/environment_name.pub"
}
...
resource "aws_key_pair" "environment_name" {
...
resource "aws_vpc" "environment_name" {
...
resource "aws_internet_gateway" "environment_name" {
...
# subnet.
resource "aws_subnet" "environment_name" {
  vpc_id     = "${aws_vpc.environment_name.id}"
...
resource "aws_route_table" "environment_name" {
  vpc_id = "${aws_vpc.environment_name.id}"

  route {
    cidr_block = "0.0.0.0/0"
    gateway_id = "${aws_internet_gateway.environment_name.id}"
  }
...
resource "aws_route_table_association" "environment_name" {
  subnet_id      = "${aws_subnet.environment_name.id}"
  route_table_id = "${aws_route_table.environment_name.id}"
}
